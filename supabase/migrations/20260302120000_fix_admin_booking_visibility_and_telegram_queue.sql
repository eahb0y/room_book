...
do $$
begin
  if to_regclass('public.telegram_bot_connections') is not null
    and to_regclass('public.telegram_booking_notifications') is not null then
    execute $fn$
    ...
    execute 'drop trigger if exists trg_enqueue_telegram_booking_notification on public.bookings';
    execute '
